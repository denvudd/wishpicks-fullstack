...
const wishId = computed(() => route.params.wishId as string)
...
// Open the specific item's modal once items are loaded
watch(fetchedItems, (items) => {
  if (!items.length) return
  const found = items.find((i) => i.id === wishId.value)
  if (found && !detailItem.value) {
    detailItem.value = found
    detailOpen.value = true
  }
}, { immediate: true })

// Sync detailItem if displayItems changes (e.g. after reserve/fulfill)
...
useSeoMeta({
  title: () => detailItem.value
    ? `${detailItem.value.title} — ${wishlist.value?.title ?? 'Wishpicks'}`
    : wishlist.value
      ? `${wishlist.value.title} — Wishpicks`
      : 'Wishpicks',
})
